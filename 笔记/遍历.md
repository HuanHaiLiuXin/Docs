...
6. Map
    ...
    - Map的实现类常用的有 HashMap,WeakHashMap,HashTable,TreeMap
        ...
        - TreeMap是一种基于红黑树的一种提供顺序访问的Map.key的排序取决于key的自然顺序(key所属类的compare方法),或构建TreeMap时候提供的Comparator实例.TreeMap也不支持同步.
            ...
            ```
7. Map深入
    1. HashMap详解参考以前保存的网页
    2. 多线程put的时候可能导致元素丢失,看源码
        ```java
        HashMap.java
        ...
        ```
    3. HashMap在高并发情况下可能引起死循环,CPU占用100%<br>
        先看源码:
        ```java
        public V put(K key, V value) {
            ****
    ...
    4. LinkedHashMap和TreeMap
        1. LinkedHashMap是1个可预知迭代顺序的Map.通过维护一个运行于所有条目的双向链表,LinkedHashMap保证了元素迭代的顺序.该迭代顺序可以是插入顺序或者是访问顺序。
            ...
            - 我们可以重写LinkedHashMap的removeEldestEntry方法,自动将**最不常被访问的对象**释放掉.
                - addEntry源码:
                    ...
                    ```
                - header.after会作为参数传入removeEldestEntry,我们可以自定义逻辑,删除**最不常被访问的对象.问题来了,header.after到底是谁?**
